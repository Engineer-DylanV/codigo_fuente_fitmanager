...
@jakarta.servlet.annotation.MultipartConfig(maxFileSize = 3 * 1024 * 1024, maxRequestSize = 4 * 1024 * 1024)
public class MobileApiServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action.toLowerCase()) {
            ...
            case "perfil_info":            handlePerfilInfo(request, response);           break;
            case "perfil_avatar":           handlePerfilAvatar(request, response);         break;
            case "perfil_foto":             handlePerfilFoto(request, response);           break;
            case "perfil_solicitar_codigo": handlePerfilSolicitarCodigo(request, response);break;
            case "perfil_cambiar_password": handlePerfilCambiarPassword(request, response);break;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setHeader("Access-Control-Allow-Origin", "*");
        String action = param(request, "action");
        if ("perfil_foto_ver".equalsIgnoreCase(action)) {
            handlePerfilFotoVer(request, response);
            return;
        }
        response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Acción GET no soportada");
    }

    // ── PERFIL (avatar, foto, sexo, seguridad) ──────────────────────────────────
    /**
     * Junta en una sola respuesta lo que en la web vive repartido entre
     * PerfilDAO.obtenerDatosCuenta / obtenerEstado / listarAvataresActivos,
     * para que la app no tenga que hacer 3 llamadas por separado.
     */
    private void handlePerfilInfo(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { sendError(res, 401, "Token inválido"); return; }
        try {
            PerfilDAO dao = new PerfilDAO();
            java.util.Map<String, String> datos = dao.obtenerDatosCuenta(uid);
            PerfilDAO.PerfilEstado estado = dao.obtenerEstado(uid);
            List<java.util.Map<String, String>> avatares = dao.listarAvataresActivos();

            StringBuilder sb = new StringBuilder("{\"ok\":true");
            sb.append(",\"email\":\"").append(esc(datos.get("email"))).append("\"");
            sb.append(",\"genero\":\"").append(esc(datos.get("genero"))).append("\"");
            sb.append(",\"avatarCodigo\":\"").append(esc(estado.getAvatarCodigo())).append("\"");
            sb.append(",\"tieneFoto\":").append(estado.isTieneFoto());
            sb.append(",\"avatares\":[");
            for (int i = 0; i < avatares.size(); i++) {
                java.util.Map<String, String> a = avatares.get(i);
                sb.append("{\"codigo\":\"").append(esc(a.get("codigo"))).append("\"")
                  .append(",\"nombre\":\"").append(esc(a.get("nombre"))).append("\"")
                  .append(",\"icono\":\"").append(esc(a.get("icono"))).append("\"")
                  .append(",\"colorInicio\":\"").append(esc(a.get("colorInicio"))).append("\"")
                  .append(",\"colorFin\":\"").append(esc(a.get("colorFin"))).append("\"}");
    ...
    private void handlePerfilAvatar(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { sendError(res, 401, "Token inválido"); return; }
        String avatar = param(req, "avatar");
        if (avatar.isEmpty()) { sendError(res, 400, "Avatar requerido"); return; }
        try {
            PerfilDAO dao = new PerfilDAO();
            if (!dao.existeAvatarActivo(avatar)) { sendError(res, 400, "El avatar seleccionado no está disponible"); return; }
            boolean ok = dao.guardarAvatar(uid, avatar);
            if (ok) res.getWriter().print("{\"ok\":true,\"message\":\"Avatar actualizado\"}");
            else sendError(res, 500, "No se pudo guardar el avatar en la base de datos");
        } catch (Exception e) { sendError(res, 500, e.getMessage()); }
    }

    private static final long MAX_FOTO_BYTES = 3L * 1024L * 1024L;

    private void handlePerfilFoto(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { sendError(res, 401, "Token inválido"); return; }
        try {
            jakarta.servlet.http.Part foto = req.getPart("foto");
            if (foto == null || foto.getSize() == 0) { sendError(res, 400, "Selecciona una imagen para continuar"); return; }
            if (foto.getSize() > MAX_FOTO_BYTES) { sendError(res, 400, "La foto no puede superar los 3 MB"); return; }
            String mime = foto.getContentType();
            String mimeNormalizado = mime == null ? "" : mime.trim().toLowerCase();
            if (!"image/png".equals(mimeNormalizado) && !"image/jpeg".equals(mimeNormalizado) && !"image/jpg".equals(mimeNormalizado)) {
                sendError(res, 400, "Solo puedes subir imágenes PNG o JPG"); return;
            }
            byte[] datos = leerYValidarImagen(foto);
            if (datos == null) { sendError(res, 400, "El archivo seleccionado no es una imagen válida"); return; }
            boolean ok = new PerfilDAO().guardarFoto(uid, mime, datos);
            if (ok) res.getWriter().print("{\"ok\":true,\"message\":\"Foto de perfil actualizada\"}");
            else sendError(res, 500, "No se pudo guardar la foto en la base de datos");
        } catch (jakarta.servlet.ServletException e) {
            sendError(res, 400, "No se recibió el archivo correctamente");
        } catch (Exception e) { sendError(res, 500, e.getMessage()); }
    }

    private byte[] leerYValidarImagen(jakarta.servlet.http.Part foto) throws IOException {
        try (java.io.InputStream entrada = foto.getInputStream()) {
            java.awt.image.BufferedImage imagen = javax.imageio.ImageIO.read(entrada);
    ...
    private void handlePerfilFotoVer(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { res.sendError(HttpServletResponse.SC_UNAUTHORIZED); return; }
        PerfilDAO.FotoPerfil foto = new PerfilDAO().obtenerFoto(uid);
        if (foto == null || foto.getDatos() == null) { res.sendError(HttpServletResponse.SC_NOT_FOUND); return; }
        res.setContentType(foto.getMime());
        res.setHeader("Cache-Control", "private, no-store, max-age=0");
        res.setHeader("X-Content-Type-Options", "nosniff");
        res.getOutputStream().write(foto.getDatos());
    }

    private void handlePerfilSolicitarCodigo(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { sendError(res, 401, "Token inválido"); return; }
        try {
            PerfilDAO perfilDao = new PerfilDAO();
            String email = perfilDao.obtenerDatosCuenta(uid).get("email");
            if (email == null || email.isEmpty()) { sendError(res, 500, "No se pudo determinar el correo de la cuenta"); return; }
            String nombre = perfilDao.obtenerDatosCuenta(uid).get("nombre");

            UsuariosDAO dao = new UsuariosDAO();
            String codigo = dao.generarCodigoCambioPassword(email, 10);
            if (codigo == null) { sendError(res, 500, "No se pudo generar el código"); return; }
            boolean enviado = EmailService.enviarCodigoCambioPassword(email, nombre != null ? nombre : "", codigo);
            if (!enviado) { sendError(res, 500, "No se pudo enviar el correo: " + EmailService.getUltimoError()); return; }
            res.getWriter().print("{\"ok\":true,\"message\":\"Enviamos un código de verificación a tu correo. Vence en 10 minutos.\"}");
        } catch (Exception e) { sendError(res, 500, e.getMessage()); }
    }

    private void handlePerfilCambiarPassword(HttpServletRequest req, HttpServletResponse res) throws IOException {
        Integer uid = TOKEN_STORE.get(param(req, "token"));
        if (uid == null) { sendError(res, 401, "Token inválido"); return; }
        String codigo = param(req, "codigo");
        String password = req.getParameter("password");
        String confirmacion = req.getParameter("confirmPassword");
        if (!codigo.matches("[0-9]{6}")) { sendError(res, 400, "Ingresa el código de seis dígitos enviado a tu correo"); return; }
        if (password == null || password.length() < 8 || password.length() > 100) {
            sendError(res, 400, "La nueva contraseña debe tener entre 8 y 100 caracteres"); return;
        }
        if (!password.equals(confirmacion)) { sendError(res, 400, "Las contraseñas no coinciden"); return; }
        try {
            String email = new PerfilDAO().obtenerDatosCuenta(uid).get("email");
            UsuariosDAO dao = new UsuariosDAO();
            UsuariosDAO.ResultadoVerificacion resultado = dao.verificarCodigoSinConsumir(email, codigo);
            if (resultado != UsuariosDAO.ResultadoVerificacion.OK) { sendError(res, 400, mensajeCodigo(resultado)); return; }
            boolean actualizada = dao.actualizarPassword(email, password);
            if (actualizada) res.getWriter().print("{\"ok\":true,\"message\":\"Tu contraseña fue actualizada correctamente\"}");
            else sendError(res, 500, "No se pudo actualizar la contraseña");
        } catch (Exception e) { sendError(res, 500, e.getMessage()); }
    }
