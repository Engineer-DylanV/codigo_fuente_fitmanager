...
public class GestionUsuariosAdminServlet extends HttpServlet {
    ...
    private Resultado cambiarEstadoUsuario(Connection con, HttpServletRequest request, boolean activar) throws SQLException {
        ...
        // Un cliente con membresia comprada no se puede desactivar
        if (!activar) {
            Resultado bloqueo = validarMembresiaComprada(con, id, "desactivar");
    ...
    Resultado eliminarUsuario(Connection con, HttpServletRequest request) throws SQLException {
        ...
        Resultado bloqueo = validarMembresiaComprada(con, id, "eliminar");
    ...
    /**
     * Valida que el usuario no tenga una membresia comprada. Un cliente con
     * membresia vigente o con compras registradas no se puede eliminar ni
     * desactivar.
     *
     * @return null si la operacion esta permitida, o el Resultado con el
     * mensaje de bloqueo.
     */
    private Resultado validarMembresiaComprada(Connection con, int id, String operacion) throws SQLException {

        // Membresia vigente (vencimiento >= hoy)
        String sqlMembresia = "SELECT u.nombre, u.apellido, m.tipo, u.vencimiento "
                + "FROM usuarios u "
                + "JOIN membresias m ON u.id_membresias = m.id_membresias "
                + "WHERE u.id_usuarios = ? AND u.vencimiento IS NOT NULL AND u.vencimiento >= CURDATE()";
            ...
            try (java.sql.ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    return new Resultado(false,
                            "No se puede " + operacion + " al cliente " + nombreUsuario
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlCompras)) {
            ...
            try (java.sql.ResultSet rs = ps.executeQuery()) {
                if (rs.next() && rs.getInt("total") > 0) {
                    ...
                    return new Resultado(false,
                            "No se puede " + operacion + " este cliente porque tiene "
