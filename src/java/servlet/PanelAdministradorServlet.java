...
public class PanelAdministradorServlet extends HttpServlet {
    ...
    List<Map<String, Object>> listarUsuariosAdmin(Connection con) throws SQLException {
        ...
        String sql = "SELECT u.id_usuarios, u.nombre, u.apellido, u.documento, u.email, "
                ...
                + "CASE WHEN u.id_membresias IS NOT NULL "
                + "AND u.vencimiento IS NOT NULL "
                + "AND u.vencimiento >= CURDATE() THEN 1 ELSE 0 END AS membresiaActiva, "
                + "(SELECT COUNT(*) FROM fact_cabecera fc WHERE fc.id_usuarios = u.id_usuarios) AS totalCompras "
                + "FROM usuarios u "
        ...
        try (PreparedStatement ps = con.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                boolean membresiaActiva = rs.getInt("membresiaActiva") == 1;
                boolean tieneCompras = rs.getInt("totalCompras") > 0;

                fila.put("membresiaActiva", membresiaActiva);
                fila.put("tieneCompras", tieneCompras);
                // Un cliente con membresia comprada no se puede eliminar ni desactivar
                fila.put("bloqueado", membresiaActiva || tieneCompras);
